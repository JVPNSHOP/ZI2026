...
# Fixed: Zivpn config format changed from "config" array to "clients" array with "password" objects
#        This resolves the connection failure (reconnecting loop) shown in user's image.
...
# Initial Config - FIXED: use clients array with password objects
if [ ! -f /etc/zivpn/config.json ]; then
    echo '{"server": ":5667", "clients": [{"password": "zi"}]}' > /etc/zivpn/config.json
fi
...
# 5. Create Web UI (Enhanced with Income and Fixed Icons - THB Updated)
...
<head>
    ...
    <style>
        ...
        .tab-active { color: #2563eb; border-bottom: 3px solid #2563eb; font-weight: 800; }
...
<div id="panel-ui" class="hidden">
    ...
    <div class="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="bg-gradient-to-br from-blue-600 to-blue-700 p-6 rounded-[2rem] text-white shadow-xl shadow-blue-100">
                <p class="text-blue-100 text-xs font-black uppercase tracking-widest mb-2">Today Sales (ယနေ့ရောင်းရငွေ)</p>
                <h3 class="text-3xl font-black" id="income-today">0 THB</h3>
            </div>
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-[2rem] text-white shadow-xl shadow-slate-200">
                <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-2">Total Sales (စုစုပေါင်းရောင်းရငွေ)</p>
                <h3 class="text-3xl font-black" id="income-total">0 THB</h3>
            </div>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl scale-in">
        ...
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-black uppercase text-slate-400 mb-2 px-1">Price (THB)</label>
                    <input type="number" id="in-pr" placeholder="50" class="w-full p-4 border rounded-2xl bg-slate-50 outline-none font-bold">
                </div>
            </div>
...
<script>
    ...
    function login() {
        if(document.getElementById('adm-u').value === ADM_U && document.getElementById('adm-p').value === ADM_P) {
            ...
            fetchUsers();
        } else { alert('Login Failed! အချက်အလက်များ ပြန်စစ်ပါ။'); }
    }
    ...
    function calculateIncome() {
        ...
        let todayTotal = 0; let grandTotal = 0;
        users.forEach(u => {
            const price = parseInt(u.price) || 0;
            grandTotal += price;
            if(u.created_at === today) { todayTotal += price; }
        });
        document.getElementById('income-today').innerText = todayTotal.toLocaleString() + ' THB';
        document.getElementById('income-total').innerText = grandTotal.toLocaleString() + ' THB';
    }
    ...
    async function save() {
        ...
        if(!id) {
            document.getElementById('alert-msg').innerHTML = \`
                <div class="flex justify-between"><span>IP Address:</span><span class="font-black text-blue-600">\${data.ip}</span></div>
                <div class="flex justify-between"><span>Password:</span><span class="font-black text-blue-600">\${data.password}</span></div>
                <div class="flex justify-between"><span>Expired:</span><span class="font-black">\${data.expired}</span></div>
                <div class="flex justify-between"><span>Status:</span><span class="px-2 bg-green-100 text-green-600 rounded-lg text-xs font-black uppercase">\${data.status}</span></div>
            \`;
            toggleAlert(true);
            lucide.createIcons();
        }
    ...
    function copy(text) {
        ...
        toast.className = "fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full text-sm font-bold shadow-2xl z-[100] animate-bounce";
    ...
    function render() {
        ...
        if(filtered.length === 0) {
            g.innerHTML = '<div class="col-span-full py-20 text-center text-slate-400 font-bold"><i data-lucide="folder-open" class="mx-auto mb-4 opacity-20" size="48"></i> No accounts found here</div>';
        ...
        filtered.forEach(u => {
            ...
            card.innerHTML = \`
                <div class="flex justify-between items-start mb-6">
                    <div class="p-4 bg-slate-50 rounded-2xl text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600 transition"><i data-lucide="user"></i></div>
                    <span class="px-3 py-1 rounded-full text-[10px] font-black tracking-widest \${statusClass} uppercase">\${isExpired ? 'OFFLINE' : u.status}</span>
                </div>
                <h3 class="text-2xl font-black text-slate-800 mb-6 truncate">\${u.username}</h3>
                <div class="space-y-4 mb-8">
                    <div class="flex justify-between text-sm"><span class="text-slate-400 font-black uppercase text-[10px]">IP Address</span> <div class="flex items-center gap-2 font-bold">\${u.ip} <button onclick="copy('\${u.ip}')" class="text-slate-300 hover:text-blue-600"><i data-lucide="copy" size="14"></i></button></div></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-400 font-black uppercase text-[10px]">Password</span> <div class="flex items-center gap-2 font-bold">\${u.password} <button onclick="copy('\${u.password}')" class="text-slate-300 hover:text-blue-600"><i data-lucide="copy" size="14"></i></button></div></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-400 font-black uppercase text-[10px]">Expired At</span> <span class="font-black">\${u.expired}</span></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-400 font-black uppercase text-[10px]">Price</span> <span class="font-black text-blue-600">\${parseInt(u.price).toLocaleString()} THB</span></div>
                </div>
                <div class="flex items-center justify-between pt-6 border-t border-slate-50">
                    <div class="bg-orange-50 text-orange-600 px-4 py-2 rounded-xl text-xs font-black">\${days} Left</div>
                    <div class="flex gap-2">
                        <button onclick="edit('\${u.id}')" class="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition"><i data-lucide="edit-3" size="18"></i></button>
                        <button onclick="del('\${u.id}')" class="p-3 bg-red-50 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition"><i data-lucide="trash-2" size="18"></i></button>
                    </div>
                </div>
            \`;
            g.appendChild(card);
...
# 6. Python Flask API Server (Fixed Zivpn Integration) - CONFIG FORMAT FIXED
            ...
            for u in data:
                if u.get('expired') and u['expired'] != 'Unlimited':
                    try:
                        ...
                        if exp_date < now: u['status'] = 'offline'
                    except: pass
            return data
    except: return []
...
def save_db(data):
    ...
    # FIXED: Zivpn config needs clients array with password objects, not a flat "config" array
    active_passwords = [str(u['password']) for u in data if u['status'] == 'active']
    
    # Fallback to a default if no active accounts
    if not active_passwords:
        active_passwords = ["zi"]
        
    # New correct format: "clients" is an array of objects with "password" field
    config_data = {"server": ":5667", "clients": [{"password": p} for p in active_passwords]}
    with open(CONFIG_PATH, 'w') as f:
        json.dump(config_data, f, indent=4)
    
    # Restart Zivpn to apply new passwords
...
echo -e "\n${BLUE}ပြင်ဆင်ပြီးသော အချက်များ:${NC}"
echo -e "${CYAN}1. Zivpn config format ကို "config" array မှ "clients" array (password objects) သို့ ပြောင်းလဲပြီး။${NC}"
echo -e "${CYAN}2. ယခု fix သည် ချိတ်ဆက်မရခြင်း (reconnecting loop) ကို ဖြေရှင်းပေးမည်။${NC}"
echo -e "${CYAN}3. အခြား မူလအတိုင်း ထိန်းသိမ်းထားသည်။${NC}"
echo -e "${BLUE}==========================================${NC}"
