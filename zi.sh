#!/bin/bash
# Zivpn Premium Management System (Full Fixed Version)
# Author: Gemini
...
# 5. Create Web UI (Modern Interface with Fixed Tabs)
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="in-pr" placeholder="1000" class="w-full p-4 border rounded-2xl bg-slate-50 outline-none font-bold">
                    </div>
                </div>
    ...
    <script>
        ...
        async function save() {
            ...
            const data = {
                ...
                price: document.getElementById('in-pr').value || 'Free',
            ...
            if(!id) {
                document.getElementById('alert-msg').innerHTML = \`
                    <div class="flex justify-between items-center border-b pb-2"><span>IP Address:</span> <div class="flex items-center gap-2 font-black text-slate-800">\${data.ip} <button onclick="copy('\${data.ip}')" class="text-blue-500"><i data-lucide="copy" size="14"></i></button></div></div>
                    <div class="flex justify-between items-center border-b pb-2"><span>Password:</span> <div class="flex items-center gap-2 font-black text-slate-800">\${data.password} <button onclick="copy('\${data.password}')" class="text-blue-500"><i data-lucide="copy" size="14"></i></button></div></div>
                    ...
                    <div class="space-y-4 mb-10">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between items-center group/item">
                                ...
                                <div class="flex items-center gap-2 font-black text-slate-700 font-mono">\${u.ip} <button onclick="copy('\${u.ip}')" class="text-blue-400 hover:text-blue-600 opacity-0 group-hover/item:opacity-100"><i data-lucide="copy" size="14"></i></button></div>
                            </div>
                            <div class="flex justify-between items-center group/item">
                                <span class="text-slate-400 font-bold uppercase text-[10px]">Password</span>
                                <div class="flex items-center gap-2 font-black text-slate-700 font-mono">\${u.password} <button onclick="copy('\${u.password}')" class="text-blue-400 hover:text-blue-600 opacity-0 group-hover/item:opacity-100"><i data-lucide="copy" size="14"></i></button></div>
                            </div>
                            <div class="flex justify-between items-center group/item">
...
# 6. Python Flask API Server (The Heart of the System)
...
def save_db(data):
    ...
    # Important: Sync with Zivpn config.json for Authentication
    active_passwords = [u['password'] for u in data if u['status'] == 'active']
    # Ensure there's always at least one password
...
echo -e "\n${BLUE}သတိပြုရန်: အကောင့်ဖွင့်ပြီးပါက Zivpn App တွင်${NC}"
echo -e "${BLUE}သင်သတ်မှတ်ထားသော Password ဖြင့် ချိတ်ဆက်နိုင်ပါပြီ။${NC}"
echo -e "${BLUE}==========================================${NC}"
