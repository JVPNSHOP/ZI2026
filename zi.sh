...
# 5. Create Web UI (Enhanced with Income and Fixed Icons)
        ...
        async function save() {
            ...
            if(!id) {
                document.getElementById('alert-msg').innerHTML = \`
                    <div class="flex justify-between items-center border-b pb-2"><span>IP Address:</span> <div class="flex items-center gap-2 font-black text-slate-800">\${data.ip} <button onclick="copy('\${data.ip}')" class="text-blue-500 hover:scale-110 active:scale-95 transition-all"><i data-lucide="copy" size="16" class="block"></i></button></div></div>
                    <div class="flex justify-between items-center border-b pb-2"><span>Password:</span> <div class="flex items-center gap-2 font-black text-slate-800">\${data.password} <button onclick="copy('\${data.password}')" class="text-blue-500 hover:scale-110 active:scale-95 transition-all"><i data-lucide="copy" size="16" class="block"></i></button></div></div>
                ...
                card.innerHTML = \`
                    ...
                    <div class="space-y-4 mb-10">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between items-center group/item py-1">
                                <span class="text-slate-400 font-bold uppercase text-[10px]">IP Address</span>
                                <div class="flex items-center gap-2 font-black text-slate-700 font-mono">\${u.ip} <button onclick="copy('\${u.ip}')" class="text-blue-400 hover:text-blue-600"><i data-lucide="copy" size="16"></i></button></div>
                            </div>
                            <div class="flex justify-between items-center group/item py-1">
                                <span class="text-slate-400 font-bold uppercase text-[10px]">Password</span>
                                <div class="flex items-center gap-2 font-black text-slate-700 font-mono">\${u.password} <button onclick="copy('\${u.password}')" class="text-blue-400 hover:text-blue-600"><i data-lucide="copy" size="16"></i></button></div>
                            </div>
                            ...
                            <div class="flex justify-between items-center py-1">
                                ...
                                <span class="font-black text-blue-600 font-mono">\${parseInt(u.price).toLocaleString()} MMK</span>
                            </div>
                        </div>
                    </div>
...
def save_db(data):
    ...
    # Important Fix: Ensure Zivpn config receives ONLY the list of passwords
    ...
    # Restart with delay and force check
...
# IPTables for UDP (Ensure correct mapping)
iptables -t nat -F
...
echo -e "${CYAN}1. Zivpn ချိတ်မရခြင်းအား အောင်မြင်စွာ ပြင်ဆင်ပြီး။${NC}"
echo -e "${CYAN}2. ဝင်ငွေစာရင်း (Today & Total Income) ထည့်သွင်းပြီး။${NC}"
echo -e "${CYAN}3. Copy ခလုတ်များ မြင်သာအောင် ပြုပြင်ပြီး။${NC}"
echo -e "${BLUE}==========================================${NC}"
